...
export default function EffEditableSelector(props:IEffEditableSelectorProps) {
    const {
        id, options, placeholder = '请选择', onChange, clear = true, searchAble = false, onSearch, size = 'large',
    } = props;
    ...
    return (
        <div
            className={`${isHover || isEditing ? 'show-status' : 'eff-editable-selector'}`}
            onMouseEnter={() => setIsHover(true)}
            onMouseLeave={() => setIsHover(false)}
        >

            <Select
                onClear={response.handlerClearSelection}
                size={size}
                ...
                value={chosenId}
                onBlur={response.handleSelectBlur}

...
export default function EffEditableInput(props:IEffInputProps) {
    const {
        value, placeholder = '请输入', fontSize = '24px', fontWeight = 500, onChange, errMsg = '请输入', isRequired = false, className,
    } = props;
    ...
    const response = {
        ...
        valueChanged: (theValue?:string) => {
            setEditValue(theValue);
            if (isRequired && !theValue) {
                setIsShowError(true);
            }
            if (theValue) {
                setIsShowError(false);
    ...
    return (
        <div
            className={`${isHover || isEditing ? 'input-status' : 'eff-editable-input'} ${className}`}

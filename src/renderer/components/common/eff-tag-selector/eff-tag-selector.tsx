...
function Tag(props:any) {
    const {
        name, id, color, selected, onChange,
    } = props;
    ...
    return (
        <div className="eff-tag" onClick={response.handleTagClicked}>
            ...
            {selected && <img alt="checked" src={IconTagCheck} width={14} />}
        </div>
    );
}
function TagSelectDlg(props: IEffTagSelectorProps) {
    ...
    const ui = {
        uiTags: availableTags.map((item) => (
            <Tag
                key={item.id}
                onChange={response.handleTagStatusChange}
                selected={chosen && chosen.includes(item.id)}
                name={item.name}
                id={item.id}
                color={item.color}
            />
    ...
    return (
        <div className="eff-tag-selector">
            ...
            <Input
                onChange={(e:any) => response.handleSearchKeyChange(e.target.value)}
                placeholder="搜索标签"

...
export default function ChangeEmailDlg(props: {
    ...
    return (
        <Modal footer={null} title={null} closable={false} visible={props.visible}>
            <div className="change-email-dlg">
                ...
                <Form  requiredMark={false} className="change-email-form" labelCol={{span:4}} colon={false} form={emailForm}>
                    <Form.Item validateStatus={passwordValidate} help={passwordErrMsg} label={'登录密码'} name={"password"} rules={[{ required: true, message: '请输入密码' }]}>
                        <Input onFocus={response.clearError} type={"password"} style={{width:inputWidth}}/>
                    </Form.Item>
                    <Form.Item  help={emailErrMsg} validateStatus={emailValidate} label={"新邮箱"} name={"email"} rules={[{ required: true, message: '请输入邮箱' }, {type:'email', message:'请输入正确的邮箱'}]}>
                        <div className="d-flex   align-center">
                            ...
                            {emailTimes == 1 && <span className="sent-msg ml20">未收到? <span onClick={response.getToken} className="cursor-pointer action">重新发送</span></span>}
                            {emailTimes == 2 && <span className="sent-msg ml20">已发送</span>}
                        </div>
                    </Form.Item>
                    <Form.Item validateStatus={tokenValidate} help={tokenErrMsg} label={'验证码'} name={'token'} rules={[{ required: true, message: '请输入验证码' }]} >
                        <Input onFocus={response.clearError} style={{width:inputWidth}}/>
                    </Form.Item>
                ...
                <div className="self-flex-end mb20">
                    <EffButton onClick={response.closeDlg} type={"line"}  round={true} key={"cancel"} text={"取消"}/>
                    <EffButton onClick={response.handleSubmit}  className="ml10" round={true} type={"filled"}  key={"ok"} text={"确定"}/>
                </div>
            </div>

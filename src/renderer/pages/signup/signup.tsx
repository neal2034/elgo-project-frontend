    ...
    return (
        <div className="signup">
            <div className="content">
                ...
                {!isEmailSent && !isUserExist && (
                    <Form form={signupForm} className="signup-form mt20">
                        ...
                        <Form.Item
                            name="email"
                            className="mt20"
                            rules={[{ required: true, message: '请输入邮箱地址' }, { type: 'email', message: '请输入有效的邮箱' }]}
                        >
                            <Input placeholder="请输入邮箱地址，将作为登录账号" />
                        </Form.Item>
                        <EffButton onClick={response.nextStep} className="mb20 mt20" text="下一步" type="filled" round width={330} />
                    </Form>
                ) }

                { isEmailSent && (
                    <div style={{ minHeight: '200px' }} className="signup-form mt20 d-flex-column">
                        <span className="success-msg mb20">
                            账号激活邮件已发送至邮箱
                            {sentEmail}
                            ,请前往激活
                        </span>
                        <div className="d-flex resent">
                            <span>未收到邮件？</span>
                            {' '}
                            <span onClick={response.resentSignupEmail} className="resent-btn">{isResent ? '已重新发送' : '重新发送'}</span>
                        </div>
                ...
                {isUserExist
                        && (
                            <Form form={newOrgForm} className="signup-form mt20">
                                ...
                                <EffButton onClick={response.goCreateNewOrg} className="mb20 mt20" text="下一步" type="filled" round width={330} />
                            </Form>
                        )}
            </div>

...
export default function AddReqClazzDlg(props:IReqClazzDlgProps) {
    ...
    const response = {
        confirmAdd: (reqClazzName:string) => {
            onConfirm(reqClazzName);
            setClazzName('');
        },
    };
    return (
        <div className="add-req-clazz-dlg d-flex-column">
            ...
            <div className="mt20 d-flex justify-end">
                <EffButton onClick={onCancel} width={80} round text="取消" key="cancel" type="line" />
                <EffButton
                    onClick={() => response.confirmAdd(clazzName!)}
                    disabled={!clazzName}
                    className="ml10"
                    width={80}
                    round

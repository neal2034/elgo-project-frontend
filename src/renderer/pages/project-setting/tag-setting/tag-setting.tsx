    ...
    return (
        <div>
            ...
            <div className="d-flex align-center mt20">
                {tags.map((item:any) => (
                    <ElgoTag
                        onDel={() => response.goDelTag(item)}
                        onEdit={() => response.goEditTag(item)}
                        className="mr10"
                        editable
                        delAble
                        key={item.id}
            ...
            <TagDialog
                ...
                onAdd={response.onAddTag}
                onEdit={response.onEditTag}
            />
            <EffConfirmDlg className="mt40" visible={confirmDelDlgVisible}>
                <div className="d-flex-column">
                    <div className="d-flex-column">
                        <span>
                            确定将标签“
                            {willDelTag && willDelTag!.name}
                            ”放入回收站
                        </span>
                    </div>
                    <div className="mt10 d-flex justify-end">
                        <EffButton onClick={() => setConfirmDelDlgVisible(false)} round key="cancel" text="取消" />
                        <EffButton onClick={response.confirmDelTag} className="mr10 ml10" type="filled" key="confirm" text="确定" round />
                    </div>
                </div>
            </EffConfirmDlg>
        </div>

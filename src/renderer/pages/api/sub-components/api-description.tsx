            ...
            {isEdit ? (
                ...
                : (
                    <div onMouseEnter={() => setHoverOnDes(true)} onMouseLeave={() => setHoverOnDes(false)} className="d-flex">
                        {description
                            ? <span>{description}</span> : <span className="add-des-btn cursor-pointer" onClick={() => setIsEdit(true)}>添加简介</span>}
                        {(hoverOnDes && !!description)
                            ? (
                                <div onClick={() => { setIsEdit(true); setHoverOnDes(false); }}>
                                    <img alt="edit" className="cursor-pointer ml10" src={ImgEdit} width={14} />
                                </div>
                            ) : null}
                    </div>
                )}

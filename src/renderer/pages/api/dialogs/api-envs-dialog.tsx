        ...
        envContentForm: (
            <div className="d-flex-column">
                ...
                <div className="mt20 self-flex-end">
                    <EffButton onClick={handler.cancelAddEnv} round key="cancel" text="取消" />
                    <EffButton onClick={handler.confirmAddEnv} className="ml10" round type="filled" key="ok" text="确定" />
                </div>
            </div>
        ),
                    ...
                    <div key={item.id} className="d-flex mb10 justify-between env-item">
                        ...
                        <div className="d-flex">
                            <div onClick={() => handler.goEditEnv(item)} className="cursor-pointer">
                                <img alt="edit" src={IconEdit} width={14} />
                                <span className="ml5">编辑</span>
                            </div>
                            <div onClick={() => handler.delEnv(item)} className="ml10 cursor-pointer">
                                <img alt="del" src={IconDel} width={14} />
                                <span className="ml5">删除</span>
                            </div>
                        </div>
                    </div>
                ))}
                <EffButton onClick={handler.handleAddEnv} className="btn-add self-flex-end mr40" round type="filled" key="add" text="添加" />
            </div>),

        noEnvContent: (
            <div className="no-env">
                ...
                <EffButton onClick={handler.handleAddEnv} className="btn-add" round type="filled" key="add" text="添加" />
            </div>),
    };
            ...
            <EffConfirmDlg className="mt40" visible={confirmDelDlgVisible}>
                <div className="d-flex-column">
                    <div className="d-flex-column">
                        <span>
                            确定将环境“
                            {willDelEnv && willDelEnv.name}
                            ”放入回收站
                        </span>
                    </div>
                    <div className="mt10 d-flex justify-end">
                        <EffButton onClick={() => setConfirmDelDlgVisible(false)} round key="cancel" text="取消" />
                        <EffButton onClick={handler.confirmDelEnv} className="mr10 ml10" type="filled" key="confirm" text="确定" round />
                    </div>
                </div>
            </EffConfirmDlg>
        </Modal>

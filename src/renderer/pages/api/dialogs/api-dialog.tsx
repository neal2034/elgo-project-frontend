...
export default function ApiDialog(props: IApiDlgProps) {
    const {
        visible, closeDlg, mode, parentId, editItem = {}, collections,
    } = props;
    ...
    const response = {
        ...
        // 选中需要保存的树节点
        handleSelectApiCollection: (paramSelectedKeys:any) => {
            setErrorChooseCollection(false);
            setSelectedKeys(paramSelectedKeys);
        },
    ...
    const ui = {
        footArea: [
            <EffButton onClick={closeDlg} round key="cancel" text="取消" />,
            <EffButton onClick={response.handleConfirm} round type="filled" key="ok" text="确定" />,
        ],
    };
